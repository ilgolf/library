...
public class Heap<E> {
    ...
    private static final int DEAFULT_CAPACITY = 10; // 최소(기본) 용적 크기

    private int size; // 요소 크기
    ...
    public Heap(Comparator<? super E> comparator) {
        this.array = new Object[DEAFULT_CAPACITY];
        this.size = 0;
    ...
    */
    ...
    public void  add(E value) {
        ...
        // 용적이 꽉 찼있을 경우 용적을 두 배로 늘려준다.
        if(size + 1 == array.length) {
            resize(array.length * 2);
        }

        siftUp(size + 1, value);   // 가장 마지막에 추가 되는 위치와 넣을 값(타켓)을 넘겨줌
    ...
    // 삽입 할 객체의 Comparable을 이용항ㄴ sift-up
    @SuppressWarnings("unchecked")
     ...
     */
     ...
     private void siftDownComparator(int idx, E target, Comparator<? super E> comp) {
        ...
        // 왼쪽 자식 노드의 인덱스가 요소의 개수보다 작을 때 까지 반복
         while((child = getLeftChild(parent)) <= size) {
             ...
             * 재배치 할 노드는 자식과 비교해야하므로 child와 childVal을
             * 오른쪽 자식으로 바꿔준다.
             */ 
     ...
     private void siftDownComparable(int idx, E target) {
         ...
         if(array.length > DEAFULT_CAPACITY && size < array.length / 4) {
             resize(Math.max(DEAFULT_CAPACITY, array.length / 2));
         }
     }
